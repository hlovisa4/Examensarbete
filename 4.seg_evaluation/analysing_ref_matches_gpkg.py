...
for d in [1.5]: # np.linspace(0, 6, 100):
    for method in ["ff3d_full", "ff3d_tile", "ff3d_ref", "chm"]:
        ...
        print(f"average height match: {np.mean(gdf_matches[f"{method}_height_ratio"])}, standard deviation: {np.std(gdf_matches[f"{method}_height_ratio"])}")


        id_to_species = {1: "Pine", 2: "Spruce", 3: "Birch", 7: "Ädel", 11: "Dead" }
        gdf["Species_name"] = gdf["Species"].map(id_to_species)

        species_accuracy = gdf.groupby("Species_name")[f"{method}_matched"].mean() * 100
        ...
        gdf_with_height = gdf[gdf['H_TLS'].notna()].copy()
        gdf_with_height['height_class'] = pd.cut(gdf_with_height['H_TLS'], bins=bins, labels=labels)    
