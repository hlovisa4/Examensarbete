...
%------------------------------------------------------------------------
    ...
    rootTLSData = 'C:/Users/digit/Downloads/Examensarbete/Results/its';
    rootResults = 'C:/Users/digit/Downloads/Examensarbete/Results/';
    % Filenames
    filenameTLS = 'instance192_TLS_ALS_norm.las';

    % create parallel pool
    p = gcp('nocreate'); 

    if isempty(p)
        % No pool found, create one
        parpool('local', 6);
    else
        % Pool already exists, tell the user
        fprintf('Parallel pool already running. Using existing pool.\n');
    end
    ...
    %%% 4. DETECT TREES and CLASSIFY STEM POINTS and CROWN POINTS
      % Saving the classified point clouds into Results folder
        ...
        %rmdir(fullfile(rootTLSData, 'tmp'), 's');
      ...
      % Construct merged and classified point cloud.
        ...
        lastools(['txt2las64 -i "',rootResults,'/pointclouds/stem/*.ply" -merged -parse xyzu -set_scale 0.0001 0.0001 0.0001 -set_classification 3 -o stempoints.las']); 
        lastools(['txt2las64 -i "',rootResults,'/pointclouds/canopy/*.ply" -merged -parse xyzu -set_scale 0.0001 0.0001 0.0001 -set_classification 4 -o canopypoints.las']);

        lastools(['lasmerge64 -i stempoints.las canopypoints.las -odir "',rootTLSData,'" -o classified.laz']);
