    ...
    for i = 1:size(filenames,1)
        try
            ...
            try
                ...
                disp(size(pc.Location))
                pc = pointCloud(pc.Location(abs(normals(:,3)) < 0.050,:));
                disp(size(pc.Location))
                pc = pcdenoise(pc,'NumNeighbors',30);
                disp("Finished step 1")
            catch ME
                disp(ME.message)
                rethrow(ME)
            end
            ...
            for k = minDistance
                ...
                for j = 1:numClusters
                    ...
                    disp(pcCluster.Count)
                    disp(abs(diff([pcCluster.ZLimits(1) pcCluster.ZLimits(2)])))

                    if (pcCluster.Count >= 100 && abs(diff([pcCluster.ZLimits(1) pcCluster.ZLimits(2)])) > 0.7) == true
                        disp("Klarade det")
            ...
            disp("Done with candidate stem point clusters")
            pctrees = pctrees(~cellfun('isempty',pctrees));
            fprintf("Found %i candidate stem point clusters\n", length(pctrees))
            ...
            if ~isempty(pctrees)
                ...
                fprintf("Detected %i potential stems\n", length(cylmodels));
                    ...
                    for j = 1:size(cylmodels,1)
                        ...
                        if ~isempty(pc.Location)
                            ...
                            if ~isempty(pc.Location)
                                ...
                                for k = 1:length(hstep)-1 %For each slice:
                                    fprintf("Slice %i \n", k)
                                    ...
                                    cylcount = 0;
                                    for q = 1:numdiverges  %For the number of diverging stems:
                                        ...
                                        if ~isempty(tmp.Location)
                                            ...
                                            if ~isempty(tmp.Location)
                                                ...
                                                fprintf("points vertical: %i \n", tmp.Count)

                                                if ~isempty(tmp.Location)
                                                    tmp = pcdenoise(tmp,'NumNeighbors',40);
                                                    fprintf("pointsnumneighbors: %i \n", tmp.Count)
                                                    ...
                                                    for m = 1:numClusters
                                                        ...
                                                        fprintf("pcClustercount: %i \n", pcCluster.Count)


                                                        % Apply RANSAC-cylinder filtering. Store cylinder parameters for later use.
                                                        if pcCluster.Count >= 40 %%%OBS:här har jag ändrat från 40
                                                            ...
                                                            params_before = length(paramlist);
                                                            try
                                                                TF = isoutlier(paramlist(:,7));
                                                                paramlist(TF,:) = []; inlierlist(TF,:) = [];
                                                            catch
                                                            ...
                                                            fprintf("Theta: %f, radius: %f, points: %i, diff %f \n", theta, clustcylmodel.Radius, mean(inlierlist), locdiff)

                                                            if (abs(theta) < 25 && clustcylmodel.Radius >= 0.015 && clustcylmodel.Radius <= 0.35 && mean(inlierlist) > 15 && locdiff < 0.3) == true %%OBS: ändrade mean(inlierlist) till 15
