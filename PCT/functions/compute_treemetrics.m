...
function treemetr = compute_treemetrics(tree_id,rootResults,h_butt,h_interval,h_layer,min_points,p)
    ...
    fprintf("The height start: %f, The tree height: %f", h_start, h_end)
    ...
    savedir = sprintf(strcat(rootResults,'/stemcurves_raw'));
    disp("saving results")
    disp(dhlist)
    if isfolder(savedir) == false, mkdir(savedir);end
    writetable(array2table(dhlist,'VariableNames',{'h','d'}),strcat(savedir,sprintf('/tree%i_stemcurve_raw.txt',tree_id)),'Delimiter','\t');
    if size(dhlist,1) > 5
        ...
        if isfolder(savedir) == false, mkdir(savedir);end
        writetable(array2table(dhlist,'VariableNames',{'h','d'}),strcat(savedir,sprintf('/tree%i_stemcurve_raw_olrm.txt',tree_id)),'Delimiter','\t');
    else
        warning('Not enough d-h-observations for tree no. %i!', tree_id);
        return
    end
    disp("Starting stem curve")
    % STEM CURVE
